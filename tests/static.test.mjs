...
test('accessibility safeguards are present',()=>{
  ...
  assert.match(forged,/\.quick-food\{flex:0 0 auto;max-width:72vw;overflow:hidden;text-overflow:ellipsis/);
...
test('food quantities support safe weight, unit and volume conversions',()=>{const nutrition=modules['js/nutrition.js'],api=new Function(nutrition+';return{foodPortionDetails,FOOD_QUANTITY_UNITS};')();assert.deepEqual(Object.keys(api.FOOD_QUANTITY_UNITS),['g','oz','piece','serving','slice','cup','tbsp','tsp','ml']);assert.equal(api.foodPortionDetails(100,'g').grams,100);assert.ok(Math.abs(api.foodPortionDetails(2,'oz').grams-56.699)<.001);assert.deepEqual(api.foodPortionDetails(2,'piece',50),{grams:100,label:'2 pieces'});assert.deepEqual(api.foodPortionDetails(1,'serving',240),{grams:240,label:'1 serving'});assert.throws(()=>api.foodPortionDetails(1,'cup',''));assert.throws(()=>api.foodPortionDetails(3000,'g'));assert.match(nutrition,/Grams per unit/);assert.match(nutrition,/aria-label="Food quantity unit"/)});
test('food macros update independently from meal completion and display as whole numbers',()=>{const nutrition=modules['js/nutrition.js'],state={meals:[{name:'Meal',ingredients:['food']}],mealIngredients:{0:['food']},mealChecks:{}},api=new Function('S','MACROS',nutrition+';return{currentNutritionTotals,selectedNutritionIngredients};')(state,{food:{kcal:321.4,p:20.6,c:31.3,f:9.8}}),totals=api.currentNutritionTotals();assert.deepEqual(totals,{kcal:321.4,p:20.6,c:31.3,f:9.8});assert.match(nutrition,/data-macro-card="today"/);assert.match(nutrition,/wholeMacro\(p\)/);assert.match(modules['js/app.js'],/meals:\[\{name:'Breakfast',ingredients:\[\]\}/)});
...
test('offline installation assets are complete',()=>{assert.equal(manifest.display,'standalone');assert.equal(manifest.start_url,'./');assert.ok(manifest.icons.some(icon=>icon.src.includes('icon.svg')));assert.match(html,/manifest\.webmanifest/);assert.match(serviceWorker,/beastmode-v18/);assert.match(serviceWorker,/css\/forged\.css/);for(const path of modulePaths)assert.match(serviceWorker,new RegExp(path.replace(/[./]/g,'\\$&')))});
